@if ($disposisi->progress_optional == 1)
	<form action="{{ route('main.kp.mahasiswa.upload-disposisi', ['progress' => 1, 'optional' => 'optional']) }}" method="post" style="display: inline;" enctype="multipart/form-data">
@endif
<table width="100%" class="table table-bordered table-striped{{ ($disposisi->progress_optional < 6 && $disposisi->progress >= 16) ? '' : formBackgroundOptional(1, 3, $disposisi) }}">
	<tbody>
		<tr>
		...
		<tr>
			...
			<td class="text-center align-middle">
				@if ($disposisi->progress_optional == 0 || ($disposisi->progress_optional < 6 && $disposisi->progress >= 16))
