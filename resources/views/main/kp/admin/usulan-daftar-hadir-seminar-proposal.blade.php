				...
				<table class="table table-bordered table-striped" id="usulan-daftar-hadir-seminar-proposal">
					...
					<tbody>
						...
						@foreach ($semua_mahasiswa as $index => $mahasiswa)
							...
							@endphp
							<tr>
								<form action="{{ route('main.kp.admin.usulan-daftar-hadir-sempro.process', ['nim' => $mahasiswa->user->nomor_induk, 'opsi' => 'accept']) }}" method="post" style="display: inline;">
								...
								<td class="align-middle text-center">
									<form action="{{ route('main.kp.admin.usulan-daftar-hadir-sempro.process', ['nim' => $mahasiswa->user->nomor_induk, 'opsi' => 'decline']) }}" method="post" style="display: inline;">
										@csrf
