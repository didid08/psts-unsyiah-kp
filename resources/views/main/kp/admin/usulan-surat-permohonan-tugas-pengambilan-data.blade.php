				...
				<table class="table table-bordered table-striped" id="usulan-surat-permohonan-tugas-pengambilan-data">
					...
					<tbody>
						...
						@foreach ($semua_mahasiswa as $index => $mahasiswa)
							...
							@endphp
							<tr>
								<form action="{{ route('main.kp.admin.usulan-sptpd.process', ['nim' => $mahasiswa->user->nomor_induk, 'opsi' => 'accept']) }}" method="post" style="display: inline;" enctype="multipart/form-data">
								...
								<td class="align-middle text-center">
									<form action="{{ route('main.kp.admin.usulan-sptpd.process', ['nim' => $mahasiswa->user->nomor_induk, 'opsi' => 'decline']) }}" method="post" style="display: inline;" enctype="multipart/form-data">
										@csrf
