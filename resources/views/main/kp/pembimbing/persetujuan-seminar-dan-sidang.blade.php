				...
				<table class="table table-bordered table-striped" id="persetujuan-diseminarkan">
					...
					<tbody>
						...
						@foreach ($semua_mahasiswa as $index => $mahasiswa)
							...
							@endphp
							<tr>
								<form action="{{ route('main.kp.pembimbing-co.persetujuan-seminar-dan-sidang.process', ['nim' => $mahasiswa->user->nomor_induk, 'type' => 'proposal']) }}" method="post" style="display: inline;">
				...
				<table class="table table-bordered table-striped" id="persetujuan-disidangkan">
					...
					<tbody>
						...
						@foreach ($semua_mahasiswa_2 as $index => $mahasiswa)
							...
							@endphp
							<tr>
								<form action="{{ route('main.kp.pembimbing-co.persetujuan-seminar-dan-sidang.process', ['nim' => $mahasiswa->user->nomor_induk, 'type' => 'buku-tga']) }}" method="post" style="display: inline;">
									<td class="align-middle text-center">{{ $index+1 }}</td>
									<td class="align-middle text-left">{{ $mahasiswa->user->nama }}</td>
									<td class="align-middle text-center">{{ $mahasiswa->user->nomor_induk }}</td>
									<td class="align-middle text-center">
				...
				<table class="table table-bordered table-striped" id="penerimaan-berkas-hasil-sidang">
					...
					<tbody>
						...
						@foreach ($semua_mahasiswa_3 as $index => $mahasiswa)
							...
							@endphp
							<tr>
								<form action="{{ route('main.kp.pembimbing-co.persetujuan-seminar-dan-sidang.process', ['nim' => $mahasiswa->user->nomor_induk, 'type' => 'lembar-pengesahan', 'opsi' => 'accept']) }}" method="post" style="display: inline;">
									<td class="align-middle text-center">{{ $index+1 }}</td>
									<td class="align-middle text-left">{{ $mahasiswa->user->nama }}</td>
									<td class="align-middle text-center">{{ $mahasiswa->user->nomor_induk }}</td>
									<td class="align-middle text-center">
								...
								<td class="align-middle text-center">
									<form action="{{ route('main.kp.pembimbing-co.persetujuan-seminar-dan-sidang.process', ['nim' => $mahasiswa->user->nomor_induk, 'type' => 'lembar-pengesahan', 'opsi' => 'decline']) }}" method="post" style="display: inline;">
										@csrf
