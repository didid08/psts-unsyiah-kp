				...
				<table class="table table-bordered table-striped" id="pengesahan-seminar-proposal">
					...
					<tbody>
						...
						@foreach ($semua_mahasiswa as $index => $mahasiswa)
							...
							@endphp
							<tr>
								<form action="{{ route('main.kp.koor-prodi.pengesahan-sempro.process', ['nim' => $mahasiswa->user->nomor_induk, 'opsi' => 'accept']) }}" method="post" style="display: inline;">
								...
								<td class="align-middle text-center">
									<form action="{{ route('main.kp.koor-prodi.pengesahan-sempro.process', ['nim' => $mahasiswa->user->nomor_induk, 'opsi' => 'decline']) }}" method="post" style="display: inline;">
										@csrf
