				...
				<table class="table table-bordered table-striped" id="penetapan-sk-penguji-seminar-proposal">
					...
					<tbody>
						...
						@foreach ($semua_mahasiswa as $index => $mahasiswa)
							...
							@endphp
							<tr>
								<form action="{{ route('main.kp.koor-prodi.penetapan-sk-penguji-sempro.process', ['nim' => $mahasiswa->user->nomor_induk, 'opsi' => 'accept']) }}" method="post" style="display: inline;">
								...
								<form action="{{ route('main.kp.koor-prodi.penetapan-sk-penguji-sempro.process', ['nim' => $mahasiswa->user->nomor_induk, 'opsi' => 'decline']) }}" method="post" style="display: inline;">
									<td class="align-middle text-center">
										@csrf
