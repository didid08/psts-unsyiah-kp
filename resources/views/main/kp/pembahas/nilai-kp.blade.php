...
@endif
	...
	<h5>NILAI KERJA PRAKTEK<br><br>DARI PEMBAHAS</h5>
	<br><br>
	<div style="text-align: left; margin-bottom: 5em;">
		...
		<table width="100%">
			...
			<tr>
				...
				<td style="width: 1%;">:</td>
				<td>
					{{ $mahasiswa->data->firstWhere('name', 'pembahas')->content }}
				</td>
			</tr>
			<tr>
	...
	<form action="{{ route('main.kp.pembahas.isi-nilai-kp.simpan', ['nim' => $mahasiswa->nomor_induk]) }}" method="post">
		@csrf
	<div style="text-align: left;">
		...
		<table width="100%" id="nilai-kerja-praktek">
			...
			<tr>
				...
				@else
				<td>
					<input type="number" name="materi" placeholder="Masukkan nilai" value="{{ $mahasiswa->data->where('name', 'nilai-materi-pembahas')->count() > 0 ? $mahasiswa->data->firstWhere('name', 'nilai-materi-pembahas')->content : '' }}">
				</td>
				<td>
					<input type="number" name="penulisan" placeholder="Masukkan nilai" value="{{ $mahasiswa->data->where('name', 'nilai-penulisan-pembahas')->count() > 0 ? $mahasiswa->data->firstWhere('name', 'nilai-penulisan-pembahas')->content : '' }}">
				</td>
				<td>
					<input type="number" name="penguasaan" placeholder="Masukkan nilai" value="{{ $mahasiswa->data->where('name', 'nilai-penguasaan-pembahas')->count() > 0 ? $mahasiswa->data->firstWhere('name', 'nilai-penguasaan-pembahas')->content : '' }}">
				</td>
				<td>
					<input type="number" name="sikap" placeholder="Masukkan nilai" value="{{ $mahasiswa->data->where('name', 'nilai-sikap-pembahas')->count() > 0 ? $mahasiswa->data->firstWhere('name', 'nilai-sikap-pembahas')->content : '' }}">
				</td>
				@endif
			</tr>
		</table>
	...
	<div style="float: right; margin-top: 5em; text-align: left;">
		...
		Pembahas <br><br><span style="margin-left: 5em;">dto</span><br><br>
		<b><u>{{ $mahasiswa->data->firstWhere('name', 'pembahas')->content }}</u></b><br>
		<b>NIP. {{ \App\User::firstWhere('nama', $mahasiswa->data->firstWhere('name', 'pembahas')->content)->nomor_induk }}</b>
	</div>
