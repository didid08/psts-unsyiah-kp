				...
				<table class="table table-bordered table-striped" id="penilaian-kp">
					...
					<tbody>
						...
						@foreach ($semua_mahasiswa as $index => $mahasiswa)
							...
							@endphp
							<tr>
								<form action="{{ route('main.kp.pembahas.penilaian-kp.process', ['nim' => $mahasiswa->user->nomor_induk]) }}" method="post" style="display: inline;">
									...
									<td class="align-middle text-center">
										@if (isset($nilai_materi_pembahas->$mhsId))
											Sudah terisi (<a href="{{ route('main.kp.pembahas.isi-nilai-kp', ['nim' => $mahasiswa->user->nomor_induk]) }}" class="text-green">Edit</a>)
										@else
											<a href="{{ route('main.kp.pembahas.isi-nilai-kp', ['nim' => $mahasiswa->user->nomor_induk]) }}" class="btn btn-outline-success"><i class="fa fa-edit mr-2"></i>Isi</a>
										@endif
									</td>
									<td class="align-middle text-center">
