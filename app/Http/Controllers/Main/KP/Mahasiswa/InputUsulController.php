...
class InputUsulController extends MainController
{
    public function view()
    {
        ...
        return $this->customView('kp.mahasiswa.input-usul', [
            'nav_item_active' => 'kp',
            'subtitle' => 'Input Usul KP',
    ...
    public function process(Request $request)
    {
        ...
        $validator_errors = [
            ...
            'judul-kp.required'           => 'Judul TGA tidak boleh kosong',
        ...
        foreach ($input as $index => $value) {
            if (in_array($index, ['foto', 'spp', 'krs', 'transkrip-sementara']))
            {
        ...
        if (array_key_exists('spp', $input) && array_key_exists('krs', $input) && array_key_exists('transkrip-sementara', $input) && array_key_exists('foto', $input)) {
            Disposisi::where('user_id', User::myData('id'))->update([
                'progress' => 2
            ]);
            return redirect(route('main.kp.disposisi'))->with('success', 'Data anda berhasil disimpan');            
        }
